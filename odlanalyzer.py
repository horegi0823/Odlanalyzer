...
def delete(odl_path):
    ...
    result = [["User behavior", "file_status", "file_path", "file_name", "file_sha1", "user_sid", "deleted_file_name", "timestamp"]]
    ...
    for idx in range(cnt): 
        ...
        try:
            tmp = eval(df.loc[df_FileRealPath_DiskLookup_idx, "Params_Decoded"])[2]
            user_sid = tmp.split("\\")[2]
            deleted_file = tmp.split('\\')[-1]
        except:
            continue
        ...
        result.append([user_behavior, file_status, file_path, file_name, file_sha1, user_sid, deleted_file, Delete_Complete_timestamp])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description = 'OneDrive User Behavior analyzer', formatter_class=argparse.RawTextHelpFormatter)
