...
are not displayed. This can be toggled with the -d option.
    ...
    parser.add_argument('-g', '--general_keystore_path', help='Path to general.keystore (if not in odl_folder)')#추가
    ...
    if args.obfuscationstringmap_path:
        obfuscation_map_path = os.path.join(args.obfuscationstringmap_path, "ObfuscationStringMap.txt")
    else:
    ...
    if args.general_keystore_path:#추가
        keystore_path = os.path.join(args.general_keystore_path, "general.keystore")#추가
    else:
    ...
    try:
        ...
        csv_f = open(csv_file_path, 'w', newline = "" , encoding='UTF8')#newline 추가, https://the-dev.tistory.com/22
